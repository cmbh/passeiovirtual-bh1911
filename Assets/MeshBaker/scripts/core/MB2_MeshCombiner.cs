...
namespace DigitalOpus.MB.Core{
	...
	*/
	...
	//		public Rect obUVRect = new Rect(0f,0f,1f,1f);
		...
		MB_DynamicGameObject instance2Combined_MapGet(GameObject key){
			return _instance2combined_map[key];
		}

		void instance2Combined_MapAdd(GameObject key, MB_DynamicGameObject dgo){
			_instance2combined_map.Add(key, dgo);
		}

		void instance2Combined_MapRemove(GameObject key){
			_instance2combined_map.Remove(key);
		}

		bool instance2Combined_MapTryGetValue(GameObject key, out MB_DynamicGameObject dgo){
		...
		bool instance2Combined_MapContainsKey(GameObject key){
	...
	//			_mesh.MarkDynamic();			
		...
		bool _addToCombined(GameObject[] goToAdd, GameObject[] goToDelete,bool disableRendererInSource){
			...
			for (int i = 0; i < _goToAdd.Length; i++){
				...
				if(!instance2Combined_MapContainsKey(_goToAdd[i]) || Array.Find<GameObject>(_goToDelete,o => o == _goToAdd[i])){
					...
					if (_goToAdd[i] != null){
						...
						dgo.lightmapTilingOffset = r.lightmapTilingOffset;
			...
			for (int i = mbDynamicObjectsInCombinedMesh.Count - 1; i >= 0;i--){
				if (mbDynamicObjectsInCombinedMesh[i]._beingDeleted){
					instance2Combined_MapRemove(mbDynamicObjectsInCombinedMesh[i].go);
	...
	//				}				
		...
		public void RebuildPrefab(GameObject prefabRoot){
			...
			this.buildSceneMeshObject(rootGO,_mesh,true);		
