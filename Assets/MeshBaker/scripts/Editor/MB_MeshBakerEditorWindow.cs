...
public class MB_MeshBakerEditorWindow : EditorWindow
{
	...
	GUIContent autoGenerateGUIContent = new GUIContent("Auto Generate Bakers", "Generates MeshBakers in the scene based on the groupings in the report.");
	...
	void addLODToSelected(){
		...
		for (int i = 0; i < newMomObjs.Count;i++){
			if (newMomObjs[i].GetComponent<MB2_LOD>() == null){
				newMomObjs[i].AddComponent<MB2_LOD>();
				numAdded++;
			}
		}
